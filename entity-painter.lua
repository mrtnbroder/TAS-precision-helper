...
function painter.PaintOutput(node)
  ...
  node.inventory = node.inventory and node.inventory.valid and node.inventory or
                     entity.get_inventory(
      node.is_crafter and defines.inventory.assembling_machine_output or node.is_furnace and
        defines.inventory.furnace_result or defines.inventory.chest)
  ...
  if node.bottom_right then
    ...
    node.bottom_right = {
      ...
      id = rendering.draw_text {
        ...
        target = {
          x = entity.bounding_box.right_bottom.x - 0.5,
...
function painter.PaintLab(node)
  ...
  local inventory = node.inventory
  if inventory == nil or research == nil then
    return
  end
  ...
  if not researching_speed then
    researching_speed = entity.prototype.researching_speed
  end
  ...
  local color = time > storage.settings.lab_yellow_swap and WHITE or time <= storage.settings.lab_red_swap and RED or
                  YELLOW
  ...
  if not node.bottom_left then
    node.bottom_left = {
      ...
      id = rendering.draw_text {
        ...
        surface = entity.surface,
        target = {
          x = entity.bounding_box.left_top.x,
          y = entity.bounding_box.right_bottom.y - 0.5
        }, -- left bottom
        color = color
      }
  ...
  if not node.top_right then
    node.top_right = {
      ...
      id = rendering.draw_text {
        ...
        surface = entity.surface,
        target = {
          x = entity.bounding_box.right_bottom.x - 0.85,
          y = entity.bounding_box.left_top.y
        }, -- right top
        color = WHITE
      }
    }
...
function painter.PaintCraftable(node)
  ...
  if not node.crafting_inventory or not node.crafting_inventory_refresh or
    not (node.crafting_inventory_refresh + 9 > tick) or not node.crafting_inventory.valid then
  ...
  if node.recipe == nil or node.crafting_inventory == nil then
    return
  end
  ...
  local color =
    time_60 <= storage.settings.crafting_red_swap and RED or time_60 <= storage.settings.crafting_yellow_swap and YELLOW or
      WHITE
...
function painter.PaintFurnaceCraftable(node)
  ...
  local color = entity.is_crafting() and WHITE or count > 1 and YELLOW or RED
...
function painter.PaintBurn(node)
  ...
  for stack, count in pairs(fuel.get_contents()) do
    storage.fuel_value[stack] = storage.fuel_value[stack] or game.item_prototypes[stack].fuel_value
    fuel_remain = fuel_remain + storage.fuel_value[stack] * count
  end
  ...
  local color = burner_time_raw <= storage.settings.burn_red_swap and RED or use_tick and YELLOW or WHITE
...
function painter.PaintCycle(node)
  if node.is_furnace and not storage.settings.cycle_furnace or node.is_miner and not storage.settings.cycle_miner then
    return
  end
  ...
  local ticks_left = not entity.is_crafting() and 0 or
                       math.ceil(60 * (1 - entity.crafting_progress) * (node.recipe.energy / entity.crafting_speed))
...
function painter.ClearPaint()
  if not storage.settings.cycle or not storage.settings.cycle_furnace or not storage.settings.cycle_miner then
    for _, node in pairs(storage.cycle_nodes) do
      if not storage.settings.cycle or node.is_furnace and not storage.settings.cycle_furnace or node.is_miner and
        not storage.settings.cycle_miner then
        if node.top_left then
          rendering.destroy(node.top_left.id)
        end
        node.top_left = nil
  ...
  if not storage.settings.crafting then
    for _, node in pairs(storage.craftable_nodes) do
      if node.bottom_left then
        rendering.destroy(node.bottom_left.id)
      end
      if node.top_right then
        rendering.destroy(node.top_right.id)
      end
      node.bottom_left = nil
      node.top_right = nil
    end
...
local function EntityBuilt(data)
  ...
  if not entity and entity.type == "entity-ghost" then
    return
  end

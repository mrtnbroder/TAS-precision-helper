...
local function update_speed_boost()
  ...
  local box = _entity and _entity.valid and _entity.bounding_box and {
    left_top = {
      x = math.floor(_entity.bounding_box.left_top.x),
      y = math.floor(_entity.bounding_box.left_top.y)
    },
    right_bottom = {
      x = math.ceil(_entity.bounding_box.right_bottom.x),
      y = math.ceil(_entity.bounding_box.right_bottom.y)
    }
  }
  if _entity and box and box.left_top.x <= _position.x and box.left_top.y <= _position.y and box.right_bottom.x >=
    _position.x and box.right_bottom.y >= _position.y then -- color change
...
local function draw_reachable_entities()
  if not storage.player_info[1].refs.settings.reachable.state then
    return
  end
...
local function build_gui(player_index)
  ...
  main_frame.location = {
    x = settings.global[settings_prefix .. "x"].value,
    y = settings.global[settings_prefix .. "y"].value
  }
  ...
  do
    ...
    title_bar.add {
      type = "sprite",
      sprite = "t-tas-helper_icon"
    }
    title_bar.add {
      type = "label",
      style = "frame_title",
      caption = " TAS precision",
      ignored_by_interaction = true
    }
    title_bar.add {
      type = "empty-widget",
      style = "t_tas_helper_title_bar_draggable_space",
      ignored_by_interaction = true
    }
    refs.toggle_options_button = title_bar.add {
      type = "sprite-button",
      style = "frame_action_button",
      sprite = "t_tas_helper_settings_icon_white",
      hovered_sprite = "t_tas_helper_settings_icon_black",
      clicked_sprite = "t_tas_helper_settings_icon_black"
    }
  ...
  do
    ...
    local function create_setting_flow(settings, name)
      ...
      local checkbox = flow.add {
        type = "checkbox",
        caption = {"gui-caption." .. name},
        state = setting(name),
        name = "show_" .. name,
        tooltip = {"gui-tooltip." .. name}
      }
      flow.add{
        type = "empty-widget"
      }.style.horizontally_stretchable = true
      local yellow = flow.add {
        type = "textfield",
        style = "very_short_number_textfield",
        tooltip = {"gui-tooltip." .. name .. "-yellow-swap"},
        text = setting(name .. "-yellow-swap"),
        numeric = true,
        allow_negative = false,
        name = name .. "_yellow_swap"
      }
      local red = flow.add {
        type = "textfield",
        style = "very_short_number_textfield",
        tooltip = {"gui-tooltip." .. name .. "-red-swap"},
        text = setting(name .. "-red-swap"),
        numeric = true,
        allow_negative = false,
        name = name .. "_red_swap"
      }
    ...
    settings.add {
      type = "checkbox",
      caption = "Highlight speed boost",
      state = setting("speed_boost"),
      name = "speed_boost",
      tooltip = {"gui-tooltip.highlight-speedboost"}
    }
    ...
    settings.reachable_range.add {
      type = "checkbox",
      caption = "Highlight reachable",
      state = setting("reachable"),
      name = "show_reachable",
      tooltip = {"gui-tooltip.highlight-reachable"}
    }
    settings.reachable_range.add{
      type = "empty-widget"
    }.style.horizontally_stretchable = true
    settings.reachable_range.add {
      type = "textfield",
      style = "very_short_number_textfield",
      text = setting("reachable-range"),
      numeric = true,
      allow_negative = true,
      name = "textfield",
      tooltip = {"gui-tooltip.reachable-range"}
    }
    ...
    settings.add {
      ...
      state = setting("output"),
      name = "show_output",
      tooltip = {"gui-tooltip.output"}
    }
    ...
    settings.cycle_flow.add {
      type = "checkbox",
      caption = "Show cycle",
      state = setting("cycle"),
      name = "show_cycle",
      tooltip = {"gui-tooltip.cycle"}
    }
    settings.cycle_flow.add{
      type = "empty-widget"
    }.style.horizontally_stretchable = true
    settings.cycle_flow.add {
      type = "checkbox",
      caption = "furnaces",
      state = setting("cycle_furnace"),
      name = "show_cycle_furnace",
      tooltip = {"gui-tooltip.cycle-furnace"}
    }
    settings.cycle_flow.add {
      type = "checkbox",
      caption = "miners",
      state = setting("cycle_miner"),
      name = "show_cycle_miner",
      tooltip = {"gui-tooltip.cycle-miner"}
    }
    ...
    settings.furnace_flow.add {
      type = "checkbox",
      caption = "Show furnace craftable",
      state = setting("furnace_craftable"),
      name = "show_furnace_craftable",
      tooltip = {"gui-tooltip.furnace-craftable"}
    }
    ...
    settings.skip_tick.add {
      type = "label",
      caption = "Skip tick [img=info]: ",
      tooltip = "The number of ticks between processing\nLower numbers gives better accuracy\nHigher numbers gives better performance",
      name = "label"
    }
    settings.skip_tick.add{
      type = "empty-widget"
  ...
  do -- controls
    local flow = main_table.add {
      type = "flow",
      direction = "vertical"
    }
    refs.btn_controls = flow
    local display_flow = flow.add {
      type = "flow",
      direction = "horizontal"
    }
    ...
    display_flow.add {
      type = "empty-widget",
      style = "t_tas_helper_horizontal_space"
    }
    local controls_flow = flow.add {
      type = "flow",
      style = "t_tas_helper_control_flow",
      direction = "horizontal"
    }
    ...
    refs.editor_button = controls_flow.add {
      type = "sprite-button",
      style = "slot_sized_button",
      tooltip = "editor",
      sprite = "t_tas_controls_editor_icon"
    }
    refs.release_button = controls_flow.add {
      type = "sprite-button",
      style = "slot_sized_button",
      tooltip = "release",
      sprite = "t_tas_controls_release_icon",
      enabled = false
    }
    refs.skip_button = controls_flow.add {
      type = "sprite-button",
      style = "slot_sized_button",
      tooltip = "skip",
      sprite = "t_tas_controls_skip_icon"
    }
  end

  do -- crafting timer, position & teleport
    local crafting_flow = main_table.add {
      type = "flow",
      direction = "vertical",
      name = "crafting_flow"
    }
    -- crafting_flow.add{ type = "label", style = "caption_label", caption = "TAS State", }
    local crafting_display_flow = crafting_flow.add {
      type = "flow",
      direction = "horizontal",
      name = "crafting_display_flow"
    }
    crafting_display_flow.add {
      type = "label",
      style = "caption_label",
      caption = "Handcrafting: "
    }
    crafting_display_flow.add {
      type = "empty-widget",
      style = "t_tas_helper_horizontal_space"
    }
    refs.crafting_timer = crafting_display_flow.add {
      type = "label",
      caption = "[0 , 0]",
      name = "crafting_timer",
      tooltip = {"gui-tooltip.crafting-timer"}
    }
    local walking_display_flow = crafting_flow.add {
      type = "flow",
      direction = "horizontal",
      name = "walking_display_flow"
    }
    walking_display_flow.add {
      type = "label",
      style = "caption_label",
      caption = "Walk target: "
    }
    ...
    refs.walking_timer = walking_display_flow.add {
      type = "label",
      caption = "[ 0 , 0] in [ 0]",
      name = "walking_timer",
      tooltip = {"gui-tooltip.walking-timer"}
    }

    local display_flow_pos = crafting_flow.add {
      type = "flow",
      direction = "horizontal"
    }
    display_flow_pos.add {
      type = "label",
      style = "caption_label",
      caption = "Position: "
    }
    ...
    refs.current_position = display_flow_pos.add {
      type = "label",
      caption = "[ 0, 0]",
      tooltip = {"gui-tooltip.position"}
    }
    ...
    refs.teleport_button = controls_flow.add {
      type = "sprite-button",
      style = "tool_button",
      tooltip = {"t-tas-helper.teleport"},
      sprite = "t_tas_controls_teleport_icon"
    }
  end
  ...
  do -- tasklist
    ...
    display_flow.add {
      type = "label",
      style = "caption_label",
      caption = {"t-tas-helper.step-list"}
    }
    display_flow.add {
      type = "empty-widget",
      style = "t_tas_helper_horizontal_space"
...
local function do_update(player_index)
  ...
  if player.controller_type == defines.controllers.character then
    ...
    refs.crafting_timer.caption = string.format("%.2f s  /  %d t,  [%d]", time, math.floor(time * 60),
      (1 - player.crafting_queue_progress) * 60 * energy)
  end
  -- player
end
...
local function update_gui_internal(player_index)
  if storage.player_info == nil then
    storage.player_info = {}
  end
...
local function toggle_gui(event)
  ...
  if not frame.visible and storage.settings_frame.visible then
    storage.settings_frame.visible = false
  end
...
local function destroy_gui(player_index)
  if storage and storage.player_info and #storage.player_info > 0 and storage.player_info[player_index].refs and
    storage.player_info[player_index].refs.main_frame then
    storage.player_info[player_index].refs.main_frame.destroy()
  end
  if storage and storage.player_info and storage.player_info[player_index] then
...
local function step_to_string(step)
  ...
  local description
  if n == "walk" then
    description = {"tas-step.description_walk", position_to_string({
      x = step[3][1],
      y = step[3][2]
    }), format_walk_direction(step)}
  elseif n == "put" or n == "take" then
    description = {"tas-step.description_" .. n, amount(step[5]), step[4], position_to_string({
      x = step[3][1],
      y = step[3][2]
    })}
  elseif n == "craft" then
  ...
  elseif n == "build" then
    return {"tas-step.description_step", step[1][1], {"tas-step.description_build", step[4], position_to_string({
      x = step[3][1],
      y = step[3][2]
    })}}
  elseif n == "mine" then
    description = {"tas-step.description_mine", position_to_string({
      x = step[3][1],
      y = step[3][2]
    }), duration(step[4])}
  elseif n == "recipe" then
    description = {"tas-step.description_recipe", step[4], position_to_string({
      x = step[3][1],
      y = step[3][2]
    })}
  elseif n == "drop" then
    description = {"tas-step.description_drop", step[4], position_to_string({
      x = step[3][1],
      y = step[3][2]
    })}
  elseif n == "filter" then
    description = {"tas-step.description_filter", step[4], position_to_string({
      x = step[3][1],
      y = step[3][2]
    })}
  elseif n == "limit" then
    description = {"tas-step.description_limit", step[4], position_to_string({
      x = step[3][1],
      y = step[3][2]
    })}
  elseif n == "tech" then
  ...
  elseif n == "priority" then
    description = {"tas-step.description_priority", step[4], step[5], position_to_string({
      x = step[3][1],
      y = step[3][2]
    })}
  elseif n == "pick" then
  ...
  elseif n == "launch" then
    description = {"tas-step.description_launch", position_to_string({
      x = step[3][1],
      y = step[3][2]
    })}
  ...
  elseif n == "rotate" then
    description = {"tas-step.description_rotate",
                   step[4] and "tas_helper_rotate_anticlockwise" or "tas_helper_rotate_clockwise", position_to_string({
      x = step[3][1],
      y = step[3][2]
    })}
  elseif n == "speed" then
  ...
  elseif n == "start" or n == "stop" or n == "pause" or n == "idle" then
    description = {"tas-step.description_misc", n == "speed" and "Game speed" or format_name(n),
                   step[3] and amount(step[3]) or ""}
  elseif n == "shoot" then
    description = {"tas-step.description_shoot", position_to_string({
      x = step[3][1],
      y = step[3][2]
    }), tostring(step[4])}
  elseif n == "throw" then
    local l = string.lower(step[4]):gsub(" ", "-")
    description = {"tas-step.description_throw", position_to_string({
      x = step[3][1],
      y = step[3][2]
    }), l}
  elseif n == "equip" then
...
local function handle_scroll(player_index, to_step)
  ...
  while math.abs(to_step - scope.index) > scope_increment do
    scope_changed = true
    if to_step - scope.index < 0 then
      scope.index = scope.index - scope_increment
    else
      scope.index = scope.index + scope_increment
    end
  end
  ...
  if data then
    for index, player_info in pairs(storage.player_info) do
      if player_info.refs and player_info.refs.release_button then
        player_info.refs.release_button.style = data.is_running and "t_tas_helper_selected_slot_sized_button" or
                                                  "slot_sized_button"
...
local function handle_walk_target_change(data)
  if data and data.target then
    ...
    if storage.current_walk_highlight_box then
      storage.current_walk_highlight_box.destroy {}
    end
    local highlight_box = game.surfaces[1].create_entity {
      name = "highlight-box",
      position = {0, 0}, -- ignored
...
local function setup_tasklist()
  ...
  if interface then
    ...
    script.on_event(remote.call("DunRaider-TAS", "get_tas_step_change_id"), handle_task_change)
    if interface.get_tas_walk_target_change_id then
      script.on_event(remote.call("DunRaider-TAS", "get_tas_walk_target_change_id"), handle_walk_target_change)
    end
    if interface.get_tas_state_change_id then
      script.on_event(remote.call("DunRaider-TAS", "get_tas_state_change_id"), handle_state_change)
    end
  end
end

script.on_event(defines.events.on_player_toggled_map_editor, function(event)
  if storage.player_info and storage.player_info[event.player_index] and storage.player_info[event.player_index].refs and
    storage.player_info[event.player_index].refs.editor_button then
    storage.player_info[event.player_index].refs.editor_button.style =
      game.players[event.player_index].controller_type == defines.controllers.editor and
        "t_tas_helper_selected_slot_sized_button" or "slot_sized_button"
  end
end)
...
script.on_load(function()
  ...
  for player_info, _ in pairs(storage.player_info) do
    local refs = storage.player_info[player_info].refs
    if interface then
      local state = interface.get_tas_state and remote.call("DunRaider-TAS", "get_tas_state") or {
        is_running = false
      }
      refs.release_button.enabled = interface.release
      refs.skip_button.enabled = interface.skip
...
script.on_event(defines.events.on_tick, function(event)
  if not game or game.players == nil or event.tick % storage.settings.skip ~= 0 or speed > 1.4 then
    return
  end
  ...
  if player.mining_state.mining then
    storage.tas_mining = storage.tas_mining or {
      pos = player.mining_state.position
    }
    local e = player.selected and
                (player.selected.prototype or game.entity_prototypes[player.selected.prototype_name]).mineable_properties
                  .mining_time or 0
    local t = string.format("%d", math.floor(
      (1 - player.character_mining_progress) * e / player.character.prototype.mining_speed * 60))
    if storage.tas_mining.id and not position_equals(storage.tas_mining.pos, player.mining_state.position) then
      rendering.destroy(storage.tas_mining.id)
      storage.tas_mining = nil
    elseif storage.tas_mining.id then
      rendering.set_text(storage.tas_mining.id, t)
    else
  ...
  do
    ...
    storage.walk_target = storage.walk_target or {
      x = 0,
      y = 0
    }
    local target_text_x, target_text_y = storage.walk_target.x > 0 and "[ %.1f," or "[%.1f,",
      storage.walk_target.y > 0 and " %.1f]" or "%.1f]"
    local target =
      string.format(target_text_x .. target_text_y, -- Complicated way of making the text not jump around too much
      storage.walk_target.x, storage.walk_target.y)
    if player.character.walking_state.walking then
      refs.walking_timer.caption = {"gui-caption.walk-target", target,
                                    math.ceil(util.distance(player.position, storage.walk_target) / 0.15)}
    else
...
local direction_strings = {"north", "northeast", "east", "southeast", "south", "southwest", "west", "northwest"}
...
local function step_to_print(step)
  if step[2] then
    ...
    elseif t == step_types.priority then
      return {"tas-print-step.priority", var.pos.x, var.pos.y, step[4], step[5],
    ...
    return {"tas-print-step." .. step[2], var.pos and var.pos.x or 0, var.pos and var.pos.y or 0, -- 1,2
    var.item or "", -- 3
    var.amount or "", -- 4
    var.dir or "", -- 5
    var.inv or "", -- 6
...
local function select_task(event)
  ...
  if storage.select_task_highlight_boxes then
    storage.select_task_highlight_boxes[1].destroy {}
    storage.select_task_highlight_boxes[2].destroy {}
  end

  local type = step[2]
  if type == "take" or type == "put" or type == "walk" or type == "build" or type == "drop" or type == "limit" or type ==
    "filter" or type == "priority" or type == "launch" or type == "recipe" or type == "rotate" or type ==
    "counter-rotate" or type == "shoot" then
    local x, y, surface = step[3][1], step[3][2], game.surfaces[1]
    storage.select_task_highlight_boxes = {surface.create_entity {
      name = "highlight-box",
      position = {0, 0}, -- ignored
    ...
    -- storage.select_task_highlight_boxes = select_task_highlight_box
...
local function toggle_settings(event)
  ...
  if location.x + math.floor((gui_width + settings_window_width) * player.display_scale) <
    player.display_resolution.width then
...
script.on_event(defines.events.on_gui_location_changed, function(event)
  for index, player_index in pairs(storage.player_info) do
    if event.element == player_index.refs.main_frame then
      has_main_frame_moved = {
        x = event.element.location.x,
        y = event.element.location.y
      }
    end
  end
end)
...
script.on_nth_tick(59, function(param1)
  if has_main_frame_moved then
    settings.global[settings_prefix .. "x"], settings.global[settings_prefix .. "y"] = {
      value = has_main_frame_moved.x
    }, {
      value = has_main_frame_moved.y
    }
...
local function handle_setting_toggled(event, skip)
  ...
  for name, element in pairs(checkboxes) do
    if element == event.element then
      if not skip then
        settings.global[settings_prefix .. name] = {
          value = element.state
        }
      end
  ...
  for name, element in pairs(integerboxes) do
    ...
    if element == event.element and
      ((name == "skip-tick" and value >= skip_tick_limit.min and value <= skip_tick_limit.max) or
        (name == "reachable-range" and value >= reachable_range_limit.min and value <= reachable_range_limit.max)) then
      if not skip then
        settings.global[settings_prefix .. name] = {
          value = element.text
        }
  ...
  for name, node in pairs(list) do
    if settings_prefix .. name == event.setting then
      if node.type == "textfield" then
        ...
        handle_painter_setting_changed({
          element = node
        }, true)
